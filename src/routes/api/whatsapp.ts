 ...
 *   WHATSAPP_VERIFY_TOKEN – secret string used during webhook setup
 *   WHATSAPP_PHONE_ID     – WhatsApp Business phone number ID
 *   LOVABLE_API_KEY       – AI Gateway key
 *
 * Features:
 *   ✅ Conversation memory (last 20 messages per user)
 *   ✅ Goals: add / complete / remove
 *   ✅ Reminders: add / remove (once + recurring)
 *   ✅ Proactive reminders — Moti sends a message when the time comes
 *   ✅ Commands: /מטרות /תזכורות /סטטוס /נקה /עזרה
 */
...
type Reminder = {
  ...
  text: string;
  kind: "once" | "recurring";
  at?: string;       // ISO datetime for once
  time?: string;     // HH:MM for recurring
  days?: string[];   // sun/mon/tue/wed/thu/fri/sat
  lastSent?: string; // prevent double-firing
};
...
// Replace with Supabase / KV for persistence across server restarts.
...
async function sendWhatsAppText(to: string, body: string): Promise<void> {
  ...
  if (!phoneId || !token) {
    console.error("Missing WHATSAPP_PHONE_ID or WHATSAPP_TOKEN");
    return;
  }
...
// Runs every 60s. Fires reminders and sends WhatsApp messages proactively.
  ...
  switch (cmd) {
    ...
    case "/goals": {
      ...
      if (open.length === 0 && done.length === 0) {
        ...
        const lines: string[] = [];
        if (open.length > 0) {
          lines.push("🎯 *מטרות פתוחות:*");
          open.forEach((g, i) => lines.push(`${i + 1}. ${g.text}`));
        }
        if (done.length > 0) {
          lines.push("\n✅ *הושלמו:*");
          done.forEach((g, i) => lines.push(`${i + 1}. ${g.text}`));
        }
    ...
    case "/status": {
      ...
      const msg =
        ...
        (openCount === 0
          ? "אין לך מטרות פתוחות. יאללה, תוסיף משהו לעשות."
          : `עוד ${openCount} מטרות מחכות לך. מתי אתה מתחיל? 😏`);
...
export const Route = createFileRoute("/api/whatsapp")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        // Start reminder scheduler on first incoming message
          ...
          try {
            ...
            for (const entry of entries) {
              ...
              for (const change of changes) {
                const messages: WAMessage[] = Array.isArray(change?.value?.messages)
                ...
                for (const msg of messages) {
                  ...
                  const result = await generateText({
                    ...
                    tools: {
                      add_goal: tool({
                        description: "הוסף מטרה חדשה.",
                        parameters: z.object({ text: z.string() }),
                        execute: async ({ text }) => {
                          session.goals.push({ id: crypto.randomUUID(), text, done: false, createdAt: new Date().toISOString() });
                          return { ok: true };
                        },
                      }),
                      complete_goal: tool({
                        description: "סמן מטרה כהושלמה.",
                        parameters: z.object({ text: z.string() }),
                        execute: async ({ text }) => {
                          const g = session.goals.find((g) => g.text.includes(text));
                          if (g) g.done = true;
                          return { ok: true };
                        },
                      }),
                      remove_goal: tool({
                        description: "מחק מטרה.",
                        parameters: z.object({ text: z.string() }),
                        execute: async ({ text }) => {
                          session.goals = session.goals.filter((g) => !g.text.includes(text));
                          return { ok: true };
                        },
                      }),
                      add_reminder: tool({
                        description: "הוסף תזכורת (חד-פעמית או חוזרת).",
                        parameters: z.object({
                          text: z.string(),
                        ...
                        execute: async (input) => {
                          session.reminders.push({ id: crypto.randomUUID(), ...input });
                          return { ok: true };
                        },
                      }),
                      remove_reminder: tool({
                        description: "מחק תזכורת.",
                        parameters: z.object({ text: z.string() }),
                        execute: async ({ text }) => {
                          session.reminders = session.reminders.filter((r) => !r.text.includes(text));
                          return { ok: true };
                        },
                      }),
                    },
