...
async function generateAiReply(
  ...
  for (const model of candidateModels(available)) {
    const res = await callGoogleGeminiModel(apiKey, model, prompt, history, text, 10_000);
    if (res.ok) {
      goodModel = model;
...
${layers.filter(Boolean).join("\n")}`;
  ...
  let cleaned = generated.content.replace(/\([a-zA-Z\s,']+\)/g, "").replace(/^[a-zA-Z\s,']+\)\s*,?/g, "").trim();
